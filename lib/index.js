...
function IsXLoggerOptionObject(option) {console.log(option);
...
function XLogger(option, useStyleParser, customLogger) {
  ...
  var _xlogger_default_methods = Object.create(Object.prototype, {
  ...
  Object.defineProperties(
    _xlogger_this,
    {
      ...
      logLevel: { get: function logLevel() { return _xlogger_define_options_log_level; } },
      setLogLevel: {
        value: function setLogLevel(value) {
          if (typeof value === 'string' && ['silent', 'error', 'warning', 'info', 'verbose'].includes(value)) {
          ...
          } else { /* Intended to be a silent fail. XLogger is used here, if verbose, instead of throwing an error.  */ _xlogger_this.IfVerbose.Error('\'value\' is not a valid LogLevel value'); }
        }
      },
    }
  );
  ...
  if (typeof customLogger === 'function') {
    _xlogger_this[Symbol.for(_xlogger_logger_symbol_key)] = customLogger;
  } else if ('console' in globalThis && typeof console.log === 'function') {
    _xlogger_this[Symbol.for(_xlogger_logger_symbol_key)] = console.log;
